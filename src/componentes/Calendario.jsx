...
export function Calendario({ tutor, asignatura, setEventos, eventos, tutorId }) {
  ...
  const guardarCita = () => {
    const nombreEstudiante = estRef.current.value.trim();
    const horas = parseInt(tiempoRef.current.value);

    if (nombreEstudiante === '' || isNaN(horas) || horas <= 0) {
      alert('Por favor, ingresa el nombre del estudiante y las horas válidas.');
      return;
    }

    const infoUsuario = { nombre_est: nombreEstudiante, tiempo: horas };
          ...
          >
            <div>
              ...
              <ul className='d-flex list-unstyled mt-2'>
                <li>
                  <button onClick={guardarCita} type='button' className='btn btn-primary'>
                    Guardar
                  </button>
                </li>
                <li>
